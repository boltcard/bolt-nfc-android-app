...
public class MainActivity extends ReactActivity {
  ...
  private void cardLogic(final Intent intent) throws Exception{
    ...
    switch (type) {
      case NTAG424DNA:
        // try {
            // String cardinfo = mCardLogic.tag424DNACardLogic(
            //   this,
            ...
            String cardDataBuilder = "Tagname: "+tagname+"\r\n"+
              "UID: "+UID+"\r\n"+
              "totalMem: "+totalMem+"\r\n"+
              "getVersion: "+Arrays.toString(getVersion)+"\r\n"+
              "vendor: "+vendor+"\r\n";
            ...
            NTAG424DNAFileSettings readSettings1 = ntag424DNA.getFileSettings(1);
            NTAG424DNAFileSettings readSettings2 = ntag424DNA.getFileSettings(2);
            // NTAG424DNAFileSettings readSettings3 = ntag424DNA.getFileSettings(3);
            
            Log.d(TAG, "read1filesize: "+readSettings1.getFileSize());
            Log.d(TAG, "read2filesize: "+readSettings2.getFileSize());
            // Log.d(TAG, "read3filesize: "+readSettings3.getFileSize());

            Log.d(TAG, "2: "+cardDataBuilder);

            //NTAG424DNAFileSettings(
              // MFPCard.CommunicationMode communicationMode, 
              // byte readAccess, 
              // byte writeAccess, 
            ...
            //SDM Access Rights. 
            //Bit 15-12: SDM Meta Read Access Rights. 
            //0x00 to 0x04 : Encrypted PICC data mirroring using the targeted AppKey 
            //0x0E : Plain PICC data mirroring 
            //0x0F : No PICC data mirroring 
            //Bit 11-8: SDM File Read Access Rights 
            //0x00 to 0x04: Targeted AppKey 
            //0x0F : No SDM for Reading 
            //Bit 3-0: SDM Counter Ret Access Rights 
            //0x00 to 0x04: Targeted AppKey 
            //0x0E : Free 
            //0x0F : No Access
            fileSettings.setSdmAccessRights(new byte[] {(byte) 0xFF, (byte) 0x12});
            ...
            Log.d(TAG, "piccOffset "+Arrays.toString(new byte[] {(byte) 38, (byte) 0, (byte) 0}));
            Log.d(TAG, "macOffset "+Arrays.toString(new byte[] {(byte) 73, (byte) 0, (byte) 0}));

            fileSettings.setPiccDataOffset(new byte[] {(byte) 38, (byte) 0, (byte) 0});
            fileSettings.setSdmMacInputOffset(new byte[] {(byte) 73, (byte) 0, (byte) 0});
            fileSettings.setSdmEncryptionOffset(null);
            fileSettings.setSdmEncryptionLength(null);
            fileSettings.setSdmMacOffset(new byte[] {(byte) 73, (byte) 0, (byte) 0});
            fileSettings.setSdmReadCounterLimit(null);

            // Log.d(TAG, "***\r\nwriteSettings: "+fileSettings.toString());
            
            ntag424DNA.changeFileSettings(2, fileSettings);
           
            // byte[] dataFromFile1 = ntag424DNA.readData(1, 0, readSettings1.getFileSize());
            // Log.d(TAG, "***\r\readDataFromFile1 : "+new BigInteger(1, dataFromFile1).toString(16));
            
            // byte[] dataFromFile2 = ntag424DNA.readData(2, 0, readSettings2.getFileSize());
            // Log.d(TAG, "***\r\readDataFromFile2 : "+new BigInteger(1, dataFromFile2).toString(16));

            // byte[] dataFromFile3 = ntag424DNA.readData(3, 0, readSettings3.getFileSize());
            // Log.d(TAG, "***\r\readDataFromFile2 : "+new BigInteger(1, dataFromFile3).toString(16));

            // NdefMessageWrapper msg = new NdefMessageWrapper(
            //   new NdefRecordWrapper(
            //     NdefRecordWrapper.TNF_UNKNOWN,
            //     new byte[], 
            //     new byte[],
            //     "lnurlw://card.yourdomain.com?p=00000000000000000000000000000000&c=0000000000000000".getBytes(Charset.forName("US-ASCII"))
            //   )
            // );
            NdefMessageWrapper msg = new NdefMessageWrapper(
              NdefRecordWrapper.createUri("lnurlw://card.yourdomain.com?p=00000000000000000000000000000000&c=0000000000000000")
            );
            ...
            WritableMap params = Arguments.createMap();
            params.putString("eventProperty", cardDataBuilder);
            sendEvent("EventReminder", params);


            // Bundle updatedProps = mReactRootView.getAppProperties();
            // Log.d(TAG, "TESTING");
            // updatedProps.putCharSequence("carddata", new String("Scanned"));
            // mReactRootView.setAppProperties(updatedProps);
            // Log.d(TAG, updatedProps == null ? "AppProperties is null" : "AppProperties is not null");
            // // updatedProps.putCharSequence("carddata", cardinfo);
            // mReactRootView.setAppProperties(updatedProps);

        // } catch (Throwable t) {
        //     showMessage(t.getMessage(), TOAST_PRINT);
        // }
        break;
      default:
        Log.d(TAG, "unknown tag type: "+type);
        showMessage("unknown tag type", TOAST_PRINT);
        break;
    }
    NxpLogUtils.save();
  }
