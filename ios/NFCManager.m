...
//              NSString *getUidHex = @"9051000000";
//              NSData *getUidCommandData = [self dataFromHexString:getUidHex];
//              NFCISO7816APDU *getUidAPDU = [[NFCISO7816APDU alloc] initWithData:getUidCommandData];
//
//              // Send the command to the tag
//              [iso7816Tag sendCommandAPDU:getUidAPDU completionHandler:^(NSData * _Nonnull responseData, uint8_t sw1, uint8_t sw2, NSError * _Nullable error) {
//                // Handle the response or error here
//                if (error) {
//                    // Handle the error here
//                    NSLog(@"getuid sendCommand Error: %@", [error localizedDescription]);
//                } else {
//                    // Log the response
//                    NSString *responseString = [responseData description];
//                    NSLog(@"getuid sendCommand Response: %@", responseString);
//                    NSLog(@"getuid sendCommand sw1: %X", sw1);
//                    NSLog(@"getuid sendCommand sw2: %X", sw2);
//
//                    // Process the response here
//                }
//              }];
